...
import { addHistory, queryHistory, removeHistory, updateHistory, queryHistoryVerboseName, queryHistoryListDisplay, queryHistoryDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateHistory(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryHistoryDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryHistoryListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryHistory, table_columns, '搜索历史-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索搜索历史" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '搜索历史-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryHistory({ ...params, sorter, filter })}
