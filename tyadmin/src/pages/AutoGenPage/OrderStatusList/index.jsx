...
import { addOrderStatus, queryOrderStatus, removeOrderStatus, updateOrderStatus, queryOrderStatusVerboseName, queryOrderStatusListDisplay, queryOrderStatusDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateOrderStatus(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryOrderStatusDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryOrderStatusListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryOrderStatus, table_columns, '订单状态-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索订单状态" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '订单状态-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryOrderStatus({ ...params, sorter, filter })}
