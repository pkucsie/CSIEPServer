...
import { addLabelType, queryLabelType, removeLabelType, updateLabelType, queryLabelTypeVerboseName, queryLabelTypeListDisplay, queryLabelTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateLabelType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryLabelTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryLabelTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryLabelType, table_columns, 'Label类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索Label类型" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'Label类型-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryLabelType({ ...params, sorter, filter })}
