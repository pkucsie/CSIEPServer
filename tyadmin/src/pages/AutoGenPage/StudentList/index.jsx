...
import { addStudent, queryStudent, removeStudent, updateStudent, queryStudentVerboseName, queryStudentListDisplay, queryStudentDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryStudentType, queryStudentTypeVerboseName} from '@/pages/AutoGenPage/StudentTypeList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateStudent(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryStudentDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryStudentListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryStudent, table_columns, '学生-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索学生" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '学生-select')
                    }
                  }}
                  selectedKeys={[]}
                >
