...
import { addCouponStatus, queryCouponStatus, removeCouponStatus, updateCouponStatus, queryCouponStatusVerboseName, queryCouponStatusListDisplay, queryCouponStatusDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCouponStatus(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryCouponStatusDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCouponStatusListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCouponStatus, table_columns, '充值方式-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索充值方式" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '充值方式-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCouponStatus({ ...params, sorter, filter })}
