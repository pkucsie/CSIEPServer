...
import { addBill, queryBill, removeBill, updateBill, queryBillVerboseName, queryBillListDisplay, queryBillDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryRechargePay, queryRechargePayVerboseName} from '@/pages/AutoGenPage/RechargePayList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateBill(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryBillListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryBill, table_columns, 'Bill-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索Bill" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'Bill-select')
                    }
                  }}
                  selectedKeys={[]}
                >
