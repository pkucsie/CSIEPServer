...
import { addUserNotice, queryUserNotice, removeUserNotice, updateUserNotice, queryUserNoticeVerboseName, queryUserNoticeListDisplay, queryUserNoticeDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryNotice, queryNoticeVerboseName} from '@/pages/AutoGenPage/NoticeList/service';
...
const TableList = () => {
        ...
        hideInSearch: true,
                        ...
                        },
                      {
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, messageidForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, messageidVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryUserNoticeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryUserNoticeListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                useEffect(() => {
                                queryNotice({all: 1}).then(value => {
                                     setMessageidForeignKeyList(value);
                                });
                                }, []);
                                const [messageidVerboseNameMap, setMessageidVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryNoticeVerboseName().then(value => {
                                    setMessageidVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryUserNotice, table_columns, '用户通知-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '用户通知-select')
                    }
                  }}
                  selectedKeys={[]}
                >
