...
import { addEmailVerifyRecord, queryEmailVerifyRecord, removeEmailVerifyRecord, updateEmailVerifyRecord, queryEmailVerifyRecordVerboseName, queryEmailVerifyRecordListDisplay, queryEmailVerifyRecordDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const base_columns = [{
        ...
        hideInSearch: true,
                      ...
                      message: '验证码类型为必填项',
                             ...
                             valueEnum:{"register":"注册","forget":"找回密码","update_email":"修改邮箱","login_auth":"登录授权"}
                        },
                      {
                             title: '发送时间',
                              ...
                              render: (text, record) => (
                                <>
                                  ...
                                  <EditOutlined title="编辑" className="icon" onClick={async () => {
                                   record.send_time = record.send_time === null ? record.send_time : moment(record.send_time);
  ...
  useEffect(() => {
    queryEmailVerifyRecordDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryEmailVerifyRecordListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryEmailVerifyRecord, table_columns, '邮箱验证码-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '邮箱验证码-select')
                    }
                  }}
                  selectedKeys={[]}
                >
