...
import { addRechargePay, queryRechargePay, removeRechargePay, updateRechargePay, queryRechargePayVerboseName, queryRechargePayListDisplay, queryRechargePayDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateRechargePay(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryRechargePayDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryRechargePayListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryRechargePay, table_columns, '充值方式-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
        ...
        request={(params, sorter, filter) => queryRechargePay({ ...params, sorter, filter })}
