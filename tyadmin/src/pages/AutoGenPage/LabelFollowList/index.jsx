...
import { addLabelFollow, queryLabelFollow, removeLabelFollow, updateLabelFollow, queryLabelFollowVerboseName, queryLabelFollowListDisplay, queryLabelFollowDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateLabelFollow(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryLabelFollowDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryLabelFollowListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryLabelFollow, table_columns, '关注Label-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索关注Label" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '关注Label-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryLabelFollow({ ...params, sorter, filter })}
