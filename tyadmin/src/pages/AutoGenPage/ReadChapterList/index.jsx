...
import { addReadChapter, queryReadChapter, removeReadChapter, updateReadChapter, queryReadChapterVerboseName, queryReadChapterListDisplay, queryReadChapterDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryRead, queryReadVerboseName} from '@/pages/AutoGenPage/ReadList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateReadChapter(value, current_id);
      hide();
        ...
        hideInSearch: true,
                        ...
                        },
                      {
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, readForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, readVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryReadChapterDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryReadChapterListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [readForeignKeyList, setReadForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryRead({all: 1}).then(value => {
                                     setReadForeignKeyList(value);
                                });
                                }, []);
                                const [readVerboseNameMap, setReadVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryReadVerboseName().then(value => {
                                    setReadVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryReadChapter, table_columns, '专栏章节-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索专栏章节" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '专栏章节-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryReadChapter({ ...params, sorter, filter })}
