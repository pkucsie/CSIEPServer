...
import { addIntegralType, queryIntegralType, removeIntegralType, updateIntegralType, queryIntegralTypeVerboseName, queryIntegralTypeListDisplay, queryIntegralTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateIntegralType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryIntegralTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryIntegralTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryIntegralType, table_columns, '积分商品类别-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索积分商品类别" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '积分商品类别-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryIntegralType({ ...params, sorter, filter })}
