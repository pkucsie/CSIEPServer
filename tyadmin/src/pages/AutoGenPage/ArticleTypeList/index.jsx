...
import { addArticleType, queryArticleType, removeArticleType, updateArticleType, queryArticleTypeVerboseName, queryArticleTypeListDisplay, queryArticleTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateArticleType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryArticleTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryArticleTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryArticleType, table_columns, '文章类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索文章类型" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '文章类型-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryArticleType({ ...params, sorter, filter })}
