...
import { addLogType, queryLogType, removeLogType, updateLogType, queryLogTypeVerboseName, queryLogTypeListDisplay, queryLogTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateLogType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryLogTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryLogTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryLogType, table_columns, '登录类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索登录类型" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '登录类型-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryLogType({ ...params, sorter, filter })}
