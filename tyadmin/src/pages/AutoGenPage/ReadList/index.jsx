...
import { addRead, queryRead, removeRead, updateRead, queryReadVerboseName, queryReadListDisplay, queryReadDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryTeacher, queryTeacherVerboseName} from '@/pages/AutoGenPage/TeacherList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateRead(value, current_id);
      hide();
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
                              const imageUrl = form.getFieldValue('detailImg');
                              return <UploadAvatar img={imageUrl}/>
                            },
                        ...
                        },
                      {
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, authorForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, authorVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryReadListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [authorForeignKeyList, setAuthorForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryTeacher({all: 1}).then(value => {
                                     setAuthorForeignKeyList(value);
                                });
                                }, []);
                                const [authorVerboseNameMap, setAuthorVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryTeacherVerboseName().then(value => {
                                    setAuthorVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryRead, table_columns, '专栏-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索专栏" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '专栏-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryRead({ ...params, sorter, filter })}
