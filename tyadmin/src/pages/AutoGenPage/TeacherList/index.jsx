...
import { addTeacher, queryTeacher, removeTeacher, updateTeacher, queryTeacherVerboseName, queryTeacherListDisplay, queryTeacherDisplayOrder} from './service';
...
const TableList = () => {
  ...
  useEffect(() => {
    queryTeacherDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryTeacherListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryTeacher, table_columns, '讲师-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索讲师" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '讲师-select')
                    }
                  }}
                  selectedKeys={[]}
                >
