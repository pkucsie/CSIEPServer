...
import { addNav, queryNav, removeNav, updateNav, queryNavVerboseName, queryNavListDisplay, queryNavDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateNav(value, current_id);
      hide();
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
  ...
  useEffect(() => {
    queryNavListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryNav, table_columns, '首页左侧菜单-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
        ...
        request={(params, sorter, filter) => queryNav({ ...params, sorter, filter })}
