...
import { addAddress, queryAddress, removeAddress, updateAddress, queryAddressVerboseName, queryAddressListDisplay, queryAddressDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateAddress(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryAddressDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryAddressListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryAddress, table_columns, '地址信息-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索地址信息" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '地址信息-select')
                    }
                  }}
                  selectedKeys={[]}
                >
