...
import { addQaType, queryQaType, removeQaType, updateQaType, queryQaTypeVerboseName, queryQaTypeListDisplay, queryQaTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateQaType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryQaTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryQaTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryQaType, table_columns, '文章类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
        ...
        request={(params, sorter, filter) => queryQaType({ ...params, sorter, filter })}
