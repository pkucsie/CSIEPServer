...
import { addCatalog, queryCatalog, removeCatalog, updateCatalog, queryCatalogVerboseName, queryCatalogListDisplay, queryCatalogDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCatalog(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryCatalogDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCatalogListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCatalog, table_columns, '课程目录信息-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索课程目录信息" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '课程目录信息-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCatalog({ ...params, sorter, filter })}
