...
import { addSysLog, querySysLog, removeSysLog, updateSysLog, querySysLogVerboseName, querySysLogListDisplay, querySysLogDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateSysLog(value, current_id);
      hide();
  ...
  useEffect(() => {
    querySysLogDisplayOrder().then(r => {
  ...
  useEffect(() => {
    querySysLogListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, querySysLog, table_columns, '系统日志-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
