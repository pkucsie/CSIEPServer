...
import { addLessonHardType, queryLessonHardType, removeLessonHardType, updateLessonHardType, queryLessonHardTypeVerboseName, queryLessonHardTypeListDisplay, queryLessonHardTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addLessonHardType({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateLessonHardType(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeLessonHardType(ids);
      hide();
  ...
  useEffect(() => {
    queryLessonHardTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryLessonHardTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryLessonHardType, table_columns, '课程难度类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索课程难度类型" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '课程难度类型-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryLessonHardType({ ...params, sorter, filter })}
