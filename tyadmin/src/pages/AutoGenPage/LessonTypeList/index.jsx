...
import { addLessonType, queryLessonType, removeLessonType, updateLessonType, queryLessonTypeVerboseName, queryLessonTypeListDisplay, queryLessonTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateLessonType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryLessonTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryLessonTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryLessonType, table_columns, '课程类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索课程类型" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '课程类型-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryLessonType({ ...params, sorter, filter })}
