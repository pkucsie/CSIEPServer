...
import { addStudentType, queryStudentType, removeStudentType, updateStudentType, queryStudentTypeVerboseName, queryStudentTypeListDisplay, queryStudentTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateStudentType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryStudentTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryStudentTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryStudentType, table_columns, '学生类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索学生类型" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '学生类型-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryStudentType({ ...params, sorter, filter })}
