...
import { addNotice, queryNotice, removeNotice, updateNotice, queryNoticeVerboseName, queryNoticeListDisplay, queryNoticeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateNotice(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryNoticeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryNoticeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryNotice, table_columns, '通知-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索通知" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '通知-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryNotice({ ...params, sorter, filter })}
