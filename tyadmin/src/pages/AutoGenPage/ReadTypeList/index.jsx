...
import { addReadType, queryReadType, removeReadType, updateReadType, queryReadTypeVerboseName, queryReadTypeListDisplay, queryReadTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateReadType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryReadTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryReadTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryReadType, table_columns, '专栏分类-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索专栏分类" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '专栏分类-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryReadType({ ...params, sorter, filter })}
