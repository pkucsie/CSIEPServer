...
import { addArticle, queryArticle, removeArticle, updateArticle, queryArticleVerboseName, queryArticleListDisplay, queryArticleDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryArticleType, queryArticleTypeVerboseName} from '@/pages/AutoGenPage/ArticleTypeList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateArticle(value, current_id);
      hide();
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
  ...
  useEffect(() => {
    queryArticleDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryArticleListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryArticle, table_columns, '文章-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索文章" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '文章-select')
                    }
                  }}
                  selectedKeys={[]}
                >
