...
import { addCommonPathConfig, queryCommonPathConfig, removeCommonPathConfig, updateCommonPathConfig, queryCommonPathConfigVerboseName, queryCommonPathConfigListDisplay, queryCommonPathConfigDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addCommonPathConfig({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCommonPathConfig(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeCommonPathConfig(ids);
      hide();
  ...
  useEffect(() => {
    queryCommonPathConfigDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCommonPathConfigListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCommonPathConfig, table_columns, '公共头部脚部配置-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索公共头部脚部配置" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '公共头部脚部配置-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCommonPathConfig({ ...params, sorter, filter })}
