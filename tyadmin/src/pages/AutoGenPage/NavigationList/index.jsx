...
import { addNavigation, queryNavigation, removeNavigation, updateNavigation, queryNavigationVerboseName, queryNavigationListDisplay, queryNavigationDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateNavigation(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryNavigationDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryNavigationListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryNavigation, table_columns, '首页左侧菜单-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索首页左侧菜单" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '首页左侧菜单-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryNavigation({ ...params, sorter, filter })}
