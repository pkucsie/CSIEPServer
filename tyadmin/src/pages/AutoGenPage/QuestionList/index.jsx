...
import { addQuestion, queryQuestion, removeQuestion, updateQuestion, queryQuestionVerboseName, queryQuestionListDisplay, queryQuestionDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryLabel} from '@/pages/AutoGenPage/LabelList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateQuestion(value, current_id);
      hide();
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
                              const imageUrl = form.getFieldValue('icon');
  ...
  useEffect(() => {
    queryQuestionDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryQuestionListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryQuestion, table_columns, '问题-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索问题" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '问题-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryQuestion({ ...params, sorter, filter })}
