...
import { addPermission, queryPermission, removePermission, updatePermission, queryPermissionVerboseName, queryPermissionListDisplay, queryPermissionDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryContentType, queryContentTypeVerboseName} from '@/pages/AutoGenPage/ContentTypeList/service';
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryPermission, table_columns, '权限-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索权限" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '权限-select')
                    }
                  }}
                  selectedKeys={[]}
                >
