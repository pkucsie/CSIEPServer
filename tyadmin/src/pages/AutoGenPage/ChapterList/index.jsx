...
import { addChapter, queryChapter, removeChapter, updateChapter, queryChapterVerboseName, queryChapterListDisplay, queryChapterDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryLesson, queryLessonVerboseName} from '@/pages/AutoGenPage/LessonList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateChapter(value, current_id);
      hide();
        ...
        hideInSearch: true,
                        ...
                        },
                      {
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, lessonForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, lessonVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryChapterDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryChapterListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [lessonForeignKeyList, setLessonForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryLesson({all: 1}).then(value => {
                                     setLessonForeignKeyList(value);
                                });
                                }, []);
                                const [lessonVerboseNameMap, setLessonVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryLessonVerboseName().then(value => {
                                    setLessonVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryChapter, table_columns, '章节-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索章节" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '章节-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryChapter({ ...params, sorter, filter })}
