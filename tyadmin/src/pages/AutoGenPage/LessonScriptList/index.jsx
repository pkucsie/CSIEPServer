...
import { addLessonScript, queryLessonScript, removeLessonScript, updateLessonScript, queryLessonScriptVerboseName, queryLessonScriptListDisplay, queryLessonScriptDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateLessonScript(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryLessonScriptDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryLessonScriptListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryLessonScript, table_columns, '课程角标类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索课程角标类型" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '课程角标类型-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryLessonScript({ ...params, sorter, filter })}
