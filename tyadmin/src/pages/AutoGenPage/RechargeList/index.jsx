...
import { addRecharge, queryRecharge, removeRecharge, updateRecharge, queryRechargeVerboseName, queryRechargeListDisplay, queryRechargeDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryRechargeAction, queryRechargeActionVerboseName} from '@/pages/AutoGenPage/RechargeActionList/service';import {queryRechargePay, queryRechargePayVerboseName} from '@/pages/AutoGenPage/RechargePayList/service';
...
const TableList = () => {
            ...
            hideInForm: true,
                        ...
                        },
                      {
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, actionForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, actionVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryRechargeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryRechargeListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                useEffect(() => {
                                queryRechargeAction({all: 1}).then(value => {
                                     setActionForeignKeyList(value);
                                });
                                }, []);
                                const [actionVerboseNameMap, setActionVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryRechargeActionVerboseName().then(value => {
                                    setActionVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryRecharge, table_columns, '充值记录-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '充值记录-select')
                    }
                  }}
                  selectedKeys={[]}
                >
