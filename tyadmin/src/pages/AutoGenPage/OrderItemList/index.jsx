...
import { addOrderItem, queryOrderItem, removeOrderItem, updateOrderItem, queryOrderItemVerboseName, queryOrderItemListDisplay, queryOrderItemDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryOrder, queryOrderVerboseName} from '@/pages/AutoGenPage/OrderList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateOrderItem(value, current_id);
      hide();
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
  ...
  useEffect(() => {
    queryOrderItemDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryOrderItemListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryOrderItem, table_columns, '订单小项-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索订单小项" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '订单小项-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryOrderItem({ ...params, sorter, filter })}
