...
import { addAnswer, queryAnswer, removeAnswer, updateAnswer, queryAnswerVerboseName, queryAnswerListDisplay, queryAnswerDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateAnswer(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryAnswerDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryAnswerListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryAnswer, table_columns, '回答-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索回答" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '回答-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryAnswer({ ...params, sorter, filter })}
