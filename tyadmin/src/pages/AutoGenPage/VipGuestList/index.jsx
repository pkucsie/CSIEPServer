...
import { addVipGuest, queryVipGuest, removeVipGuest, updateVipGuest, queryVipGuestVerboseName, queryVipGuestListDisplay, queryVipGuestDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateVipGuest(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryVipGuestDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryVipGuestListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryVipGuest, table_columns, '嘉宾-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索嘉宾" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '嘉宾-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryVipGuest({ ...params, sorter, filter })}
