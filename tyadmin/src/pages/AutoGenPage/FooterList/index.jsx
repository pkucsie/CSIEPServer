...
import { addFooter, queryFooter, removeFooter, updateFooter, queryFooterVerboseName, queryFooterListDisplay, queryFooterDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateFooter(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryFooterDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryFooterListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryFooter, table_columns, '底部配置-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索底部配置" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '底部配置-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryFooter({ ...params, sorter, filter })}
