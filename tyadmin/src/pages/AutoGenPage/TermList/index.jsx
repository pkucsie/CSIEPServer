...
import { addTerm, queryTerm, removeTerm, updateTerm, queryTermVerboseName, queryTermListDisplay, queryTermDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryChapter, queryChapterVerboseName} from '@/pages/AutoGenPage/ChapterList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateTerm(value, current_id);
      hide();
        ...
        hideInSearch: true,
                        ...
                        },
                      {
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, chapterForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, chapterVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryTermListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [chapterForeignKeyList, setChapterForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryChapter({all: 1}).then(value => {
                                     setChapterForeignKeyList(value);
                                });
                                }, []);
                                const [chapterVerboseNameMap, setChapterVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryChapterVerboseName().then(value => {
                                    setChapterVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryTerm, table_columns, '小节-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索小节" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '小节-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryTerm({ ...params, sorter, filter })}
