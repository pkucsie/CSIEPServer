...
import { addConsult, queryConsult, removeConsult, updateConsult, queryConsultVerboseName, queryConsultListDisplay, queryConsultDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateConsult(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryConsultDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryConsultListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryConsult, table_columns, '用户咨询-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索用户咨询" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '用户咨询-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryConsult({ ...params, sorter, filter })}
