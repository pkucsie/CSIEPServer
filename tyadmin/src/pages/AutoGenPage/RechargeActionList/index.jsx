...
import { addRechargeAction, queryRechargeAction, removeRechargeAction, updateRechargeAction, queryRechargeActionVerboseName, queryRechargeActionListDisplay, queryRechargeActionDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addRechargeAction({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateRechargeAction(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeRechargeAction(ids);
      hide();
  ...
  useEffect(() => {
    queryRechargeActionDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryRechargeActionListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryRechargeAction, table_columns, '充值类型-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索充值类型" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '充值类型-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryRechargeAction({ ...params, sorter, filter })}
